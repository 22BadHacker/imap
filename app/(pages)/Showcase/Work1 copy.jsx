...
const imgs = [
    ...
    {name:'Business Card', desc:['Illustrator', 'Infographie'], src: pics5, category: 'Bureautique' },
    ...
    {name:'Portfolio Website', desc:['React', 'Tailwind Css', 'Informatique'], src: pics11, category: 'Genie Civil' },
]
        ...
        <div className='w-full container h-auto max-w-[1450px] sm:px-5 px-5 mx-auto grid grid-cols-1 gap-[40px]'>
            ...
            <div className="w-full coontain relative h-auto py-[120px] z-[1]">
                <div className="flex gap-3 flex-col">
                    ...
                    <p className='font-semibold text-[15px] tracking-wider font-metro text-main2 flex gap-1 items-center'>
                ...
                <div className="column columns-2 md:columns-3 gap-2 md:gap-3 mt-12 z-20">
                    {filteredImgs.map((items, i) => (
                        <div
                            key={i}
                            className='w-full  first:mt-0 h-full relative mt-2 cursor-pointer' 
                            onClick={() => openModal(items.src)}
                        >
                            <Image
                                alt=''
                                className='w-full shadow-sm z-30 first:mt-0 h-full object-cover md:mt-3'
                                src={items.src}
                            />

                            <div className="absolute top-0 left-0 w-full h-full backdrop-blur-[20px] bg-black/20 flex items-center justify-center opacity-0 hover:opacity-100 transition-all duration-300 ease-in-out">
                                <div className="flex flex-col gap-4 items-center justify-center text-white text-center">
                                    <h1 className='md:text-[25px] text-[18px] font-semibold font-metro'>{items.name} <span className=' text-[30px]'>.</span></h1>
                                    <p className='text-[14px] font-metro flex gap-3 flex-wrap'>{Array.isArray(items.desc) ? items.desc.map((item, i) => (<span className='border-[1px] px-3 py-1 rounded-full' key={i}>{item}</span>)) : items.desc}</p>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {isModalOpen && (
                <div className="fixed top-0 left-0 w-full h-full bg-black/70  flex items-center justify-center z-50">
                    <div className="relative">
                        ...
                        <button
                            onClick={closeModal}
                            className="absolute -top-8 right-0 duration-200 hover:text-main2 text-white text-3xl font-bold"
                        >
                            <AiOutlineClose />
                        </button>
                    </div>
